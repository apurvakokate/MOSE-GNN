    ...
    do
        ...
        do
            ...
            do
                ...
                do 
                    for DATASETNAME in esol Lipophilicity
                    do
                    ...
                    for DATASETNAME in BBBP Mutagenicity hERG
                    do
                        FOLDER="EXPT-12BC-$DATASETNAME-SEED-$SEED-FOLD-$FOLD-$LAYERTYPE-$MODELTYPE-$ALGORITHM"
                        mkdir $FOLDER
                        python 1_run_EXPT_SingleChannel.py --dataset_name $DATASETNAME --seed $SEED --fold $FOLD --algorithm $ALGORITHM --layer_type $LAYERTYPE --task_type BinaryClass --model_type $MODELTYPE --size_reg 0.0 --date_tag 1225 --epochs 500 --output_dir $FOLDER > $FOLDER/EXPT-out.out 2>&1
                        # Kill any lingering Python processes
                        pkill -f 1_run_EXPT_SingleChannel.py
                        # Pause to allow memory reclamation
                        sleep 10
