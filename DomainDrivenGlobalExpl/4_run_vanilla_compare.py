...
if vanilla_dir == "None":
    ...
    if hasattr(model, 'motif_params'):  
        # Define file paths
        test_file = f"{output_dir}/{dataset_name}_vanilla_impact_withposthoc_test.csv"
        val_file = f"{output_dir}/{dataset_name}_vanilla_impact_withposthoc_validation.csv"
        train_file = f"{output_dir}/{dataset_name}_vanilla_impact_withposthoc_train.csv"

        # Save only if the file does not exist
        if not os.path.exists(test_file):
            save_csv_motif_importance(model, motif_list, [(test_mask_data, test_data, test_loader)], test_file)

        if not os.path.exists(val_file):
            save_csv_motif_importance(model, motif_list, [(val_mask_data, validation_data, val_loader)], val_file)
    ...
    if hasattr(model, 'motif_params'):  
        ...
        # Save only if the file does not exist
        if not os.path.exists(test_file):
            save_csv_motif_importance(model, motif_list, [(test_mask_data, test_data, test_loader)], test_file, vanilla_model= vanilla_model)

        if not os.path.exists(val_file):
            save_csv_motif_importance(model, motif_list, [(val_mask_data, validation_data, val_loader)], val_file, vanilla_model= vanilla_model)

        if not os.path.exists(train_file):
            save_csv_motif_importance(model, motif_list, [(train_mask_data, training_data)], train_file, vanilla_model= vanilla_model)
