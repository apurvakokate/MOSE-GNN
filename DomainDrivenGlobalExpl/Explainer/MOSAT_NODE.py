...
class GNNModel(nn.Module): 
    ...
    def forward(self, x, edge_index, batch=None, node_to_motifs = None,edge_weight = None, ignore_unknowns=False, return_logit=False):
        ...
        if self.use_ones:
            ...
            emb = self.embedding(x, edge_index, node_to_motifs)
    ...
    def embedding(self, x, edge_index, node_to_motif_tensor = None):
        for conv in self.convs:
            conv = conv.to(edge_index.device)
            x = conv(x, edge_index)
            x = torch.nn.functional.normalize(x, p=2, dim=1)
            x = F.relu(x)
        if node_to_motif_tensor is not None:
            for index_of_node_in_batch, motif_index in enumerate(node_to_motifs):
                #Perform motif level readout such that each motif in the graph has an embedding
        return x
