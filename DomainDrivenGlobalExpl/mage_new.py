...
class MAGE:
    ...
    def decode_tree(self, trees, graphs, z_tree, max_iter):
        ...
        for i, data in enumerate(tree_list):
            ...
            if data.x.shape[0] == 1:
                ...
                # pdb.set_trace()
                label_loss += self.criterion(label_pred, data.x[0].long().unsqueeze(0))
                # label_loss += self.criterion(label_pred, data.x[0])
                count_topo += 1
                count_label += 1
                count_positive += 1

                tree_pred_loss, acc_correct, acc_wrong, new_tree, _ = self.sample_tree(tree_emb, max_iter=max_iter, label = data.data.y)
            ...
            while queue:
                ...
                for j in range(length):
                    ...
                    if node[1] == -2:
                        ...
                        label_loss += self.criterion(label_pred,data.x[node[1]].unsqueeze(0))
                        count_label += 1
    ...
    def sample_tree(self, z_tree, max_iter, test=False, label=None):
        iter = 0
        queue = deque([(-1, 0)])
            ...
            for j in range(length):
                ...
                if torch.argmax(topo_pred) == 1 or node[0] == -1:                   
                    ...
                    if node[0] == -1:
                        ...
                        tree_pred_loss += self.bce_criterion(pred.view(-1), label.float())
                        #self.bce_criterion(pred.squeeze(), torch.tensor(label, device=self.device, dtype=torch.float))
                        # tree_pred_loss += self.bce_criterion(pred, torch.tensor(label, device=self.device, dtype=torch.float))
                        ...
                        if test:
                            ...
                            for i in range(len(indices[0])):
                                ...
                                if site_pair:
                                    ...
                                    tree_pred_loss += self.bce_criterion(pred.squeeze(), torch.tensor(label, device=self.device, dtype=torch.float))
                                    break
                        else:
    ...
    def decode_graph(self, tree):
        ...
        if not queue:
            ...
            # return smiles, pred[0, self.label].item(), embedding for multi_label
        while queue:
    ...
    def get_motif_mask(self):
        ...
        if os.path.exists("checkpoints/motif_selection/"+'BBBP'+"_motif_"+str(self.label)+".pt"):
            ...
            motif_filter(self.whole_dataset, 'BBBP', self.smiles_set, self.model, 2, self.device)
            selected_motif = torch.load("checkpoints/motif_selection/"+'BBBP'+"_motif_"+str(self.label)+".pt")
        ...
        input("Motif Mask learning done for each class")
